...
async function validateReservationMiddleware(req, res, next) {
    ...
    if (!isWithinInterval(dateHourReservation, { start: startInterval, end: endInterval })) return res.status(400).json({error: "Reservas permitidas apenas das 18:00 até 23:59, exceto aos domingos."});

    if (isSunday(dateHourReservation)) return res.status(400).json({error: "Reservas não são permitidas aos domingos."});
